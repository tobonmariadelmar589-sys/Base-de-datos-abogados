...
# ────────────────────────────────────────────────────────────
#  BASE DE DATOS — conexión y stored procedures
# ────────────────────────────────────────────────────────────
...
def sp_insert_cliente(d):
    with get_conn() as c: c.execute(
        "INSERT INTO clientes(codigo,tipo,nombre,documento,direccion,telefono,correo,fecha_contacto,referido_por,clasificacion,foto) VALUES(:codigo,:tipo,:nombre,:documento,:direccion,:telefono,:correo,:fecha_contacto,:referido_por,:clasificacion,:foto)", d)

def sp_update_cliente(d):
    with get_conn() as c: c.execute(
        "UPDATE clientes SET tipo=:tipo,nombre=:nombre,documento=:documento,direccion=:direccion,telefono=:telefono,correo=:correo,fecha_contacto=:fecha_contacto,referido_por=:referido_por,clasificacion=:clasificacion,foto=:foto WHERE codigo=:codigo", d)
...
def sp_insert_abogado(d):
    with get_conn() as c: c.execute(
        "INSERT INTO abogados(num_colegiatura,nombres,apellidos,especialidad,anios_exp,formacion,idiomas,tarifa_hora,disponibilidad,foto) VALUES(:num_colegiatura,:nombres,:apellidos,:especialidad,:anios_exp,:formacion,:idiomas,:tarifa_hora,:disponibilidad,:foto)", d)

def sp_update_abogado(d):
    with get_conn() as c: c.execute(
        "UPDATE abogados SET nombres=:nombres,apellidos=:apellidos,especialidad=:especialidad,anios_exp=:anios_exp,formacion=:formacion,idiomas=:idiomas,tarifa_hora=:tarifa_hora,disponibilidad=:disponibilidad,foto=:foto WHERE num_colegiatura=:num_colegiatura", d)
...
# Stored procedures — audiencias
def sp_insert_audiencia(d):
    with get_conn() as c: c.execute(
        "INSERT INTO audiencias(codigo,tipo,id_caso,fecha_hora,duracion_estimada,lugar,participantes_int,participantes_ext,proposito,resultado_esperado,resultado_real) VALUES(:codigo,:tipo,:id_caso,:fecha_hora,:duracion_estimada,:lugar,:participantes_int,:participantes_ext,:proposito,:resultado_esperado,:resultado_real)", d)

def sp_update_audiencia(d):
    with get_conn() as c: c.execute(
        "UPDATE audiencias SET tipo=:tipo,id_caso=:id_caso,fecha_hora=:fecha_hora,duracion_estimada=:duracion_estimada,lugar=:lugar,participantes_int=:participantes_int,participantes_ext=:participantes_ext,proposito=:proposito,resultado_esperado=:resultado_esperado,resultado_real=:resultado_real WHERE codigo=:codigo", d)
...
def exportar_excel(titulo, encabezados, filas, filtro_info=""):
    try:
        ...
        fill_nav  = PatternFill("solid", fgColor="1E2761")
        ...
        c.fill = fill_nav; c.alignment = centro
        ...
        for col, enc in enumerate(encabezados, 1):
            ...
            c.fill = fill_gold; c.alignment = centro; c.border = borde
        ...
        wb.save(ruta)
        messagebox.showinfo("Exito", f"Excel guardado en:\n{ruta}")
...
def exportar_pdf(titulo, encabezados, filas, filtro_info=""):
    try:
        ...
        elems.append(Paragraph("JUSTICIA & ASOCIADOS",
            ParagraphStyle("t", fontSize=16, fontName="Helvetica-Bold", textColor=navy, alignment=1, spaceAfter=4)))
        elems.append(Paragraph(titulo.upper(),
            ParagraphStyle("s", fontSize=12, fontName="Helvetica-Bold", textColor=gold, alignment=1, spaceAfter=4)))
        info = f"Exportado: {dt.now().strftime('%d/%m/%Y %H:%M')}"
        if filtro_info: info += f"   |   Filtro: {filtro_info}"
        elems.append(Paragraph(info,
            ParagraphStyle("i", fontSize=9, fontName="Helvetica", textColor=colors.grey, alignment=1, spaceAfter=10)))
        ...
        for i in range(1, len(data)):
            if i % 2 == 0:
                estilo.add("BACKGROUND",(0,i),(-1,i),colors.HexColor("#F2F4F8"))
        tabla.setStyle(estilo)
        elems.append(tabla)
        elems.append(Spacer(1, 0.4*cm))
        elems.append(Paragraph(f"Total registros: {len(filas)}",
            ParagraphStyle("p", fontSize=8, fontName="Helvetica", textColor=colors.grey, alignment=2)))
        doc.build(elems)
...
def cargar_imagen(label_widget):
    try:
        ...
        if os.path.getsize(ruta) > 5*1024*1024:
            messagebox.showerror("Archivo muy grande","La imagen no debe superar 5 MB."); return None
        ...
        messagebox.showerror("Error imagen", str(e)); return None

def mostrar_img_bytes(label_widget, datos):
    try:
        from PIL import Image, ImageTk
        if not datos:
            label_widget.config(image="", text="Sin foto"); return
...
def crear_favicon(root):
    try:
        ...
        d.rectangle([28,10,36,50], fill=(201,168,76,255))
        d.ellipse([18,10,46,24],   fill=(201,168,76,255))
        d.ellipse([12,24,30,36],   fill=(201,168,76,255))
        d.ellipse([34,24,52,36],   fill=(201,168,76,255))
...
m_tema.add_command(label="Tema Claro",  command=lambda: aplicar_tema(root,"Claro"))
m_tema.add_command(label="Tema Oscuro", command=lambda: aplicar_tema(root,"Oscuro"))
...
def lbl(p, t, r, c=0):
    tk.Label(p, text=t, font=("Calibri",11), fg="#1E2761", anchor="w").grid(
        row=r, column=c, sticky="w", padx=(0,10), pady=6)

def ent(p, r, c=1, w=28):
    e = tk.Entry(p, width=w, font=("Calibri",11), relief="solid", bd=1, bg="#FFFFFF", fg="#2D3748")
    e.grid(row=r, column=c, sticky="w", pady=6)
    return e

def combo(p, var, vals, r, c=1):
    cb = ttk.Combobox(p, textvariable=var, values=vals, width=26, state="readonly")
    cb.grid(row=r, column=c, sticky="w", pady=6)
    return cb

def botones(parent, cmds):
    colores = {
...
def make_tree(parent, cols, height=6):
    ...
    for col in cols:
        ...
        tv.column(col, width=130, anchor="center")
    sb = ttk.Scrollbar(f, orient="horizontal", command=tv.xview)
    tv.configure(xscrollcommand=sb.set)
    tv.pack(fill="x"); sb.pack(fill="x")
    return tv

def filtro_frame(parent, widgets_fn):
    f = tk.LabelFrame(parent, text=" Filtros de Exportacion ",
                      font=("Calibri",10,"bold"), fg="#1E2761", padx=10, pady=5)
...
c_fec = DateEntry(f1, width=26, date_pattern="yyyy-mm-dd", font=("Calibri",11))
...
lbl_fc = tk.Label(fp1, text="Sin foto", width=20, height=8, relief="solid", bd=1, bg="#F0F2F8")
lbl_fc.pack(pady=5)
...
f1_tipo=tk.StringVar(); f1_des=[None]; f1_has=[None]; f1_cla2=tk.StringVar()
...
filtro_frame(inner1, build_filtros1)
...
def delete1():
    ...
    if not messagebox.askyesno("Eliminar",f"Eliminar el cliente '{cod}'?\nEsta accion no se puede deshacer."): return
...
def excel1():
    ...
    exportar_excel("Clientes",["Codigo","Tipo","Nombre","Documento","Telefono","Correo","Clasificacion"],
        [(r["codigo"],r["tipo"],r["nombre"],r["documento"],
          r["telefono"] or "",r["correo"] or "",r["clasificacion"] or "") for r in rows],
...
botones(inner1,{"Guardar":save1,"Actualizar":update1,"Eliminar":delete1,"Limpiar":clear1,"Excel":excel1,"PDF":pdf1})
refresh1()
...
filtro_frame(inner2, build_filtros2)
...
def delete2():
    ...
    if not messagebox.askyesno("Eliminar",f"Eliminar el abogado '{col}'?"): return
...
botones(inner2,{"Guardar":save2,"Actualizar":update2,"Eliminar":delete2,"Limpiar":clear2,"Excel":excel2,"PDF":pdf2})
refresh2()

# ════════════════════════════════════════════════════════════════
#   MODULO 3 — CASOS LEGALES
# ════════════════════════════════════════════════════════════════
...
f3_est=tk.StringVar(); f3_ram=tk.StringVar(); f3_des3=[None]; f3_has3=[None]
...
filtro_frame(inner3, build_filtros3)
...
def save3():
    ...
    if mostrar_errores(e): return
    try: sp_insert_caso(d); messagebox.showinfo("Guardado","Caso legal guardado correctamente."); clear3(); refresh3(); refresh_combos3()
    except Exception as ex: messagebox.showerror("Error",str(ex))
...
def delete3():
    ...
    if not messagebox.askyesno("Eliminar",f"Eliminar el caso '{num}'? Esta accion es irreversible."): return
...
botones(inner3,{"Guardar":save3,"Actualizar":update3,"Eliminar":delete3,"Limpiar":clear3,"Excel":excel3,"PDF":pdf3})
refresh3()

# ════════════════════════════════════════════════════════════════
#   MODULO 4 — AUDIENCIAS Y CITAS
# ════════════════════════════════════════════════════════════════
...
f4_tip=tk.StringVar(); f4_des4=[None]; f4_has4=[None]
...
filtro_frame(inner4,build_filtros4)
...
def delete4():
    ...
    if not messagebox.askyesno("Eliminar",f"Eliminar la audiencia '{cod}'?"): return
...
botones(inner4,{"Guardar":save4,"Actualizar":update4,"Eliminar":delete4,"Limpiar":clear4,"Excel":excel4,"PDF":pdf4})
refresh4()
